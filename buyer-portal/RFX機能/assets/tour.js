/* ============================================================
   ...
   ・ページに専用ツアー（#btnTour：ProjectCreate 等）が既にある場合は何もしない。
   ・フローティングの「🧭 ガイドツアー」ボタンを右下手前（左下）に追加。
     初回アクセス時（セッション単位）は自動起動する。
   ・依存なし（vanilla JS）。共通アセットとして各RFX画面から読み込む。
   参照: phase_scope.md「画面ガイド機能（WalkMe類似）」= Phase 2 候補。本実装は
        モックアップ上のプレビュー（プロトタイプ）であり、Phase 1 の機能スコープには含めない。
   ============================================================ */
(function () {
  function ready(fn) {
  ...
  ready(function () {
    ...
    // 専用ツアーがあるページ（ProjectCreate 等）はスキップ
    ...
    if (steps.length < 2) return; // 案内する意味がない画面は出さない
