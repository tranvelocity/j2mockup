/* ============================================================
   J2 RFX機能モックアップ（Supplier Portal）— 共通レイアウト拡張
   ------------------------------------------------------------
   実共通レンダラ（../assets/layout.js の JienieLayout）に対し RFX 画面用に:
     1. openModal / closeModal 追加
     2. init() をラップ: 親リンク補正（../）・RFXメニュー注入・モーダル/検索バインド
   ============================================================ */
(function () {
  ...
  // 実レンダラのリンクは supplier-portal/ 基準。RFX画面は supplier-portal/RFX機能/ にあるため
  // BHW_*.html / eProcurement_*.html への相対リンクを ../ で親フォルダへ補正する。
  function fixParentLinks() {
    document.querySelectorAll('.header a[href], #wrapper-sidebar a[href]').forEach(function (a) {
      ...
      if (/^(BHW_|eProcurement_|CMS)/.test(href) || /^[A-Za-z0-9_]+\.html/.test(href)) a.setAttribute('href', '../' + href);
    });
  }

  // 新設メニュー「調達案件(RFX)」をサイドバー末尾に追加。
  function injectRfxMenu() {
    ...
    li.innerHTML =
      '<a href="#" class="menu-item active" title="調達案件(RFX)">' +
        '<svg viewBox="0 0 24 24"><path d="M3 3h2v18H3V3zm4 11h3v7H7v-7zm5-7h3v14h-3V7zm5 4h3v10h-3V11z"/></svg>' +
        '<span class="menu-label">調達案件(RFX)<span class="mi-new">NEW</span></span>' +
    ...
    head.addEventListener('click', function (e) {
      e.preventDefault();
      if (!document.body.classList.contains('sidebar-open')) { window.location.href = 'eProcurement_RFX_Supplier_ProjectList.html'; return; }
